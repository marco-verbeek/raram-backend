...
exports.overview_timer = async function (req, res) {
  const summonerData = await leagueJs.Summoner.gettingByName(
    req.query.name ?? process.env.DEFAULT_SUMMONER_NAME ?? 'ItsNexty')
  const accountId = summonerData['accountId']

  const timer = await db.getUserTimer([accountId])

  if (timer.rowCount === 0)
    return res.json({ 'error': 'Could not find a timer linked to this user' }).
      status(500).
      end()
...
exports.set_timer = async function (req, res) {
  ...
  if (req.body.value === undefined || isNaN(value))
    return res.json({ 'error': 'Missing or incorrect value parameter' }).
      status(500).
      end()
  ...
  if (value === 0) {
    ...
    const addedValue = (current_amount + value) > 0 ? value : current_amount *
      -1
    await db.updateTimerGames([accountId, addedValue])
  }

  return res.json({ 'success': 'The timer has successfully been updated' }).
    status(200).
    end()
}