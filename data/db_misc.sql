...
FROM raram.users;
SELECT *
FROM raram.verifications;
SELECT *
FROM raram.matches
GROUP BY match_id, account_id;
SELECT *
FROM raram.stats;
SELECT u.summoner_name, u.lp, s.games_played, s.current_winstreak, s.highest_winstreak
FROM raram.users u,
     raram.stats s
...
SELECT raram_date, raram_amount
FROM raram.users
WHERE summoner_name = 'ItsNexty';